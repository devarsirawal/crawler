...
pid_fl = PID(1e-2, 1e-3, 1e-5, setpoint=velocity)
pid_fr = PID(1e-2, 1e-3, 1e-5, setpoint=velocity)
...
while not gym.query_viewer_has_closed(viewer):
    ...
    if frame_count >= 50:
        ...
        input_fl = np.clip(pid_fl(dof_states[fl_id, 1].item()), -1e-2, 1e-2)
        input_fr = np.clip(pid_fr(dof_states[fr_id, 1].item()), -1e-2, 1e-2)
        # input_fl = pid_fl(dof_states[fl_id, 1].item())
        # input_fr = pid_fr(dof_states[fr_id, 1].item())

        print(dof_states[fl_id, 1].item())
        print(input_fl)
        print()

        actions[fl_id] = input_fl  
        actions[fr_id] = input_fr
    ...
    if 50 < frame_count < args.max_plot_time + 50:
        ...
        if args.use_torque:
            logger_vars["lf_cmd_torque"] = input_fl
            logger_vars["rf_cmd_torque"] = input_fr
        # else:
